#!/usr/bin/env node
/**
 * Add a "Sync History" quick-nav link to admin.html's header (inside
 * __bundler/template, not the manifest), matching the existing "Sync
 * Conflicts" link's exact style, placed right after it.
 */
'use strict';
const fs = require('fs');
...
const OLD_STR = '<a href="sync-conflicts.html" style="display:inline-flex;align-items:center;gap:8px;flex:0 0 auto;height:30px;padding:0 13px;border:1px solid rgba(255,255,255,.28);border-radius:4px;background:transparent;color:#C7BFB2;font-size:13px;font-weight:500;text-decoration:none;white-space:nowrap">Sync Conflicts</a>';
const NEW_STR = OLD_STR +
  '\n    <a href="sync-history.html" style="display:inline-flex;align-items:center;gap:8px;flex:0 0 auto;height:30px;padding:0 13px;border:1px solid rgba(255,255,255,.28);border-radius:4px;background:transparent;color:#C7BFB2;font-size:13px;font-weight:500;text-decoration:none;white-space:nowrap">Sync History</a>';
...
if (APPLY) {
  ...
  if (!verifyTemplate.includes('href="sync-history.html"')) throw new Error(page + ': verification FAILED -- missing sync-history.html link after write.');
