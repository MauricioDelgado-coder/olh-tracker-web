...
const OLD_1 = "          key: (f[\"Job #\"] || \"\") + \"|\" + w.code,\n          manager: this.mgrName(f[w.mgrKey])\n        });";
const NEW_1 = "          key: (f[\"Job #\"] || \"\") + \"|\" + w.code,\n          manager: this.mgrName(f[w.mgrKey]),\n          inspApproved: !!f[\"Home Inspection Approved\"]\n        });";
...
if (APPLY) {
  ...
  for (const must of ['inspApproved: !!f["Home Inspection Approved"]', 'header: "Insp. Approved"', 'r.inspApproved ? "Yes" : "No"']) {
