/**
 * Add the CCR Bonus Approval tile to index.html: a new canBonusApproval
 * permission prop, plus a new tile card right after the CCR Monthly Bonus
 * tile, matching the existing tile markup pattern exactly (blue scheme,
 * distinct from the amber CCR Monthly Bonus tile so the two are visually
...
const OLD_1 = '      canBonus: this.can("page.bonus"),\n';
const NEW_1 = OLD_1 + '      canBonusApproval: this.can("page.bonusapproval"),\n';
...
'      <a href="bonus-approval.html" style="display:flex;flex-direction:column;padding:20px 20px 22px;border:1px solid #E4DED2;border-radius:12px;background:#fff;box-shadow:0 1px 3px rgba(27,42,88,.06);transition:box-shadow 200ms cubic-bezier(.2,.6,.2,1),transform 200ms cubic-bezier(.2,.6,.2,1),border-color 200ms cubic-bezier(.2,.6,.2,1)" style-hover="box-shadow:0 12px 28px rgba(27,42,88,.14);transform:translateY(-4px);border-color:#C6D8F3">\n' +
...
'          <svg width="22" height="22" sc-camel-view-box="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9 12l2 2 4-4"></path><circle cx="12" cy="12" r="9"></circle></svg>\n' +
'        </span>\n' +
...
'        <div style="display:flex;flex-wrap:wrap;gap:6px;margin:16px 0 0">\n' +
'          <span style="height:22px;padding:0 9px;border-radius:999px;background:#EAF1FB;font-size:11px;font-weight:600;line-height:22px;color:#1B3E7C">Direct reports</span>\n' +
'          <span style="height:22px;padding:0 9px;border-radius:999px;background:#EAF1FB;font-size:11px;font-weight:600;line-height:22px;color:#1B3E7C">Approve / reject</span>\n' +
'        </div>\n' +
'        <span style="display:inline-flex;align-items:center;gap:7px;margin-top:auto;padding-top:18px;font-size:13px;font-weight:600;color:#1B3E7C">Review Submissions\n' +
'          <svg width="15" height="15" sc-camel-view-box="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 12h14"></path><path d="M13 6l6 6-6 6"></path></svg>\n' +
'        </span>\n' +
'      </a>\n' +
'      </sc-if>\n' +
...
// index.html's own convention (see patch-index-bonus-tile.js): every "/"
// becomes uppercase "\u002F", and every non-ASCII char becomes a lowercase
// \uXXXX escape.
...
if (APPLY) {
  ...
  for (const must of ['canBonusApproval: this.can("page.bonusapproval")', 'sc-if value="{{ canBonusApproval }}"', 'href="bonus-approval.html"', '>CCR Bonus Approval<']) {
