#!/usr/bin/env python3
"""
SANDBOX VARIANT -- writes ONLY to Jobs (Sandbox - SAN) / tbltB2CIKBumT6sMK.

This is a fork of dev/sync_coe_to_airtable.py with the table id swapped, built to
prove out a San Antonio tracker without touching the live OLH Jobs table
(tblqpmwtZ6i4gtogl) or anything downstream of it (walk scheduler, background
rebalance, live pages). Defaults --division to SAN since that is this sandbox's
only purpose; pass --division to point it elsewhere if it is ever reused, but at
that point just diff against the real script rather than trusting this comment.

Everything below this point is unchanged from the live script except JOBS_TABLE,
the lock file prefix, and the --division default. See dev/sync_coe_to_airtable.py
for the authoritative version and its full design notes.

    python3 dev/sync_coe_to_airtable_sandbox_san.py --out "<folder>" [--dry-run] [--skip-report]
...
def pat():
    ...
    if not v:
        die('AIRTABLE_PAT is not set. Try:\n'
            ...
            '    python3 dev/sync_coe_to_airtable_sandbox_san.py --out "<folder>"')
    return v
...
def main():
    ...
    p.add_argument('--division', default='SAN')
    ...
    if not a.dry_run:
        ...
        lock_path = os.path.join(WORK, 'sync-sandbox-%s.lock' % div)
        try:
