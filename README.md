...
step on the host — but the pages **are** built, locally, by `dev/release.sh`
before they are committed. Nothing is served from the repo root.

```
olh-tracker-web/
├── public/                        # THE PUBLISH DIRECTORY. build output — never hand-edit.
...
│   ├── export-map.json            # which export pages ship, and why one does not
│   ├── build-live-pages.js        # design export → deployable page
│   ├── live-loader.js             # inlined into each page; fetches /api/*
│   ├── whats-live.sh              # what is deployed vs. what is in this repo
...
URLs in the browser on load. That is why the top-level `fonts/` holds only a
README — the font files are inlined. No bundle contains a credential, and none
talks to Airtable directly; all go through `/api/*` exactly as described below.

Because the fonts are inlined, `public/fonts/` and `public/vendor/` are currently
referenced by **nothing**: extracting every `assets/` and `fonts/` reference from
all eight bundles (including the compressed manifest entries, which is where a
reference can hide) yields exactly one file, `assets/lennar-logo-blue.png`. They
are kept because `checkStaticRefs()` in `build-live-pages.js` fails the build if a
re-export starts asking for one, and having them already present is cheaper than
diagnosing eight broken pages. Do not read their presence as evidence they are in
use.

The build **deletes** the bundled data snapshot from every page — 900 synthetic
homesites, ~1 MB each — and injects `dev/live-loader.js` in its place. That is
what makes "no page ships fake data" a structural fact rather than a runtime
hope, and it is asserted per page, so a re-export that moves the snapshot fails
the build instead of publishing invented records.

## The eight pages

Every one is generated by `dev/build-live-pages.js` from a design export. None is
hand-edited. `public/` is build output.

| Path | Reads | Writes | What it is |
|---|---|---|---|
| `/` | — | — | Sign-in landing page and tile menu. Carries the auth module. |
| `/tracker` | `GET /api/jobs` | `POST /api/update-job` | The editable tracker: one row per homesite, 26 editable columns. The only page that writes. |
| `/completion` | `GET /api/jobs` | — | Completion Report, grouped by community. Scoped to saleable lots with a live projected date. |
| `/scheduler` | `GET /api/jobs`, `/api/walk-config` | — | Walk scheduling. |
| `/walk-calendar` | `GET /api/jobs`, `/api/walk-config` | — | Walk Schedule Export. |
| `/workload` | `GET /api/jobs`, `/api/walk-config` | — | Workload Predictor. |
| `/workload-visualizer` | `GET /api/jobs`, `/api/walk-config` | — | Workload Visualizer. |
| `/admin` | `GET /api/jobs`, `/api/users`, `/api/roles`, `/api/audit` | `POST /api/users`, `/api/roles`, `/api/invite`, `/api/set-password` | User administration. Gated on the `roster.manage` capability. |

`404.html` also ships. It is hand-written, not generated — the one page in
...
## Releasing a design change

One command. It stages the export, applies the allow-list, builds, checks, and
only then touches `public/`:

```bash
bash dev/release.sh ~/Downloads/"OLH Tracker design exploration"
```

Then read the diff it prints and:

```bash
git commit -am "Rebuild on the <date> export"
git push            # the push IS the deploy — see below
```

What the script does, and why it is one script and not a checklist:

1. **Refuses to start** if `public/` has uncommitted changes. A release has to be
   reproducible from a known commit.
2. **Stages the export** under `exports/<date>/` — `raw/` verbatim, `pages/`
   assembled from the allow-list. `exports/` is gitignored (a 2 MB
   `olh-data.js` of real homesite records is a build input, not source).
3. **Applies `dev/export-map.json`.** The export contains the same nine pages
   twice: loose `*.dc.html` design source, and `production/*.html` bundled and
   already named for deployment. The bundled ones are what the build reads —
   they carry the `__bundler/manifest` blocks. The map is the allow-list of which
   eight ship, which is what keeps `tracker-new.html` from aborting the build.
4. **Writes `exports/manifest.json`** — names, sizes and content hashes of both
   halves of the export. This *is* committed. Without it, a design change is a
   1 MB diff in a generated file with no way to tell which export produced it.
5. **Builds into a temp dir**, seeded from `public/` minus the eight generated
   pages, so a page that fails to build is *absent* rather than silently
   inherited from the last release.
6. **Runs the checks** — `check-export-errors.sh` in headless Chrome (the one
   that catches a bundler-mangled declaration, which changes no text and throws
   nothing the build can see), then `build-azure-api.js --check`.
7. **Promotes** with `rsync --delete`, and writes `public/version.json`.

`dev/verify-pages.sh` is *not* run by the script: it takes a base URL, not a
folder, and needs the site served with functions live. Run it against a deploy
preview or `netlify dev`.
...
Fetches `/version.json` from both hosts and compares to local `HEAD`, and warns
about unpushed commits or a dirty `public/`. Expect the deployed SHA to be one
behind `HEAD` — `version.json` records the commit the pages were built *on top
of*, and the commit that ships them is its child.
...
> only files copied into `public/` can be served. With `publish = "."` a 07/29
> deploy published a 3.6 MB zip of the design folder, and a later one published
> the folder itself, xlsx export of real homesite data and all. Blocking each leak
> with a forced 404 was whack-a-mole. Now a new file in the repo is private by
...
**Avoid this.** It uploads your working tree, not `main` — so it can put code
live that is in no commit, and it leaves the two hosts serving different things
with nothing in git to show it. `dev/whats-live.sh` will report the disagreement
but cannot tell you what the extra bytes were. If you do use it to get out of a
hole, commit and push the same tree immediately afterwards.

> It is also the weaker path for privacy: see the allow-list note under
...
5. **No secrets client-side.** No token, base id secret, or credential is in any
   ...
   > is whack-a-mole, and lost twice (a 3.6 MB design zip, then the design folder
   > itself with an xlsx of real homesite data). `.gitignore` was never the
   > mechanism, and cannot be: `dev/` is deliberately **committed** now, because
   > excluding it cost the repo its own build tooling — a README documenting
