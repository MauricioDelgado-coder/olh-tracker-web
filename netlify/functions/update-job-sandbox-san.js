/**
 * POST /api/update-job-sandbox-san
 *
 * SANDBOX VARIANT of update-job.js -- writes ONLY to Jobs (Sandbox - SAN) /
 * tbltB2CIKBumT6sMK. Not the live Jobs table, not reachable from any live page.
 *
 * The EDITABLE whitelist below is trimmed from the live version's 45 fields
 * down to the ones that actually exist on the sandbox table: the walk-miss
 * logging fields (Missed/Miss Reason/Miss Note/Walk Miss Reconciled), the
 * reschedule flags, and NOC Lock Date were deliberately left off the sandbox
 * table (out of scope for a tracker-only proof of concept), so they are left
 * off this whitelist too rather than accepted here and 422'd by Airtable.
 *
 * Same five defense-in-depth layers as the live endpoint -- session +
 * tracker.edit, POST-only single-record, strict allow-list, per-type
 * validation, noindex headers. See update-job.js for the full rationale.
 */
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // sandbox.edit, not tracker.edit -- see the note on sandbox.edit in
    // netlify/lib/olh-auth.js for why these are kept separate. A role that
    // holds only sandbox.edit must get a 403 here on the live endpoint too,
    // not just a hidden UI tile.
    A.requirePerm(session, 'sandbox.edit');
  } catch (err) {
    return A.fail(err);
  }
